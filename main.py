# CayteringWork Bot - Railway deployment
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    await msg.answer("👋 Добро пожаловать в CayteringWork_Bot!\n\n"
...
async def adm_approve(cb: CallbackQuery, bot: Bot):
    ...
    await cb.message.edit_text(f"✅ *{e.full_name}* одобрен.", parse_mode="Markdown")
...
async def adm_reject(cb: CallbackQuery, bot: Bot):
    ...
    async with db() as s:
        ...
        await s.commit()
        r = await s.execute(select(Employee).where(Employee.id == eid)); e = r.scalar_one_or_none()
    await cb.message.edit_text(f"❌ Заявка *{e.full_name}* отклонена.", parse_mode="Markdown")
...
async def main():
    global _Session
    print("🚀 CayteringWork Bot starting...")
    ...
    engine = create_async_engine(db_url, connect_args={"ssl": True}, pool_pre_ping=True)
    ...
    print(f"✅ Started | admins={list(SUPER_ADMIN_IDS)}")
